...
- `https://github.com/PostPlusAI/postplus-skills`: the public source repository for released marketing skills bundled with the CLI.
...
The CLI package includes its matching skills. `postplus install` installs them
for supported local agents and records their verified content. Reuse a correct
installation when switching sessions or agents; do not run a separate skills
installer.
...
If an update fails, stop and follow its reported next action; do not repeatedly
update or retry the original task. A `requires_human` result needs the user's
decision. Agents must not create a pseudo-terminal (PTY) or pipe confirmation
input to bypass it. `postplus update --yes` backs up and replaces real local
skill changes only after the user explicitly authorizes that replacement.
